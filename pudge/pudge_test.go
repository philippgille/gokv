...
import (
	...
	"github.com/philippgille/gokv/pudge"
...
func TestNonExistingDir(t *testing.T) {
	tmpDir := os.TempDir() + "/pudge"
	err := os.RemoveAll(tmpDir)
	if err != nil {
		t.Fatal(err)
	}

	options := pudge.Options{
		Path: tmpDir,
	}
	store, err := pudge.NewStore(options)
...
func createStore(t *testing.T, codec encoding.Codec) (pudge.Store, string) {
...
func generateRandomTempDbPath(t *testing.T) string {
	path, err := ioutil.TempDir(os.TempDir(), "pudge")
	if err != nil {

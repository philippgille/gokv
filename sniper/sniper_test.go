...
import (
	"github.com/philippgille/gokv/sniper"
	"io/ioutil"
	"log"
	"os"
	"testing"
...
func TestNonExistingDir(t *testing.T) {
	tmpDir := os.TempDir() + "/sniper"
	err := os.RemoveAll(tmpDir)
	if err != nil {
		t.Fatal(err)
	}

	options := sniper.Options{
		Path: tmpDir,
	}
	store, err := sniper.NewStore(options)
...
func createStore(t *testing.T, codec encoding.Codec) (sniper.Store, string) {
...
func generateRandomTempDbPath(t *testing.T) string {
	path, err := ioutil.TempDir(os.TempDir(), "sniper")
	if err != nil {

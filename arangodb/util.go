...
// the first uint64 indicates the length in bytes. if the first element is 0, empty array
...
// if input length is 0 or 1, will return nil, as minimum length is 2 (len+data)
...
// as a key value database, we do not support prefix queries
// possible support for prefix queries can be added through a second indexed field.
// for now, i will convert a string into arango-key safe hex bytes hash
